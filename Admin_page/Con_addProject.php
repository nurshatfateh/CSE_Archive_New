<?php
  ...
  $RelevantProjects = $_POST['RelevantProjects'];
	...
	if($conn->connect_error){
		...
		/// $image_Path = "projectimg/".basename($Get_image_name);  

		$stmt = $conn->prepare("insert into projects(ProjectName,Academicyear,NoofStudent, StudentID, Supervisors,Member-1Name,Member-2Name,Member-3Name,Member-4Name,Member-5Name,CommenceDate,CompletionDate, ProjectTitle,ProjectDomain,ProjectSynopsis,Proposal, Prototype,Budget,URLofFrontend,GithubLink,ProjectFile,RelevantProjects) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
		$stmt->bind_param("helloprojects", $ProjectName, $Academicyear, $NoofStudent,$StudentID,$Supervisors,$Member_1Name,$Member_2Name,$Member_3Name,$Member_4Name, $Member_5Name,$CommenceDate,$CompletionDate,$ProjectTitle,$ProjectDomain,$ProjectSynopsis,$Proposal,$Prototype,$Budget,$URLofFrontend,$GithubLink,$ProjectFile,$RelevantProjects);
